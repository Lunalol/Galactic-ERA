...
//
			this.GALACTIC_GOALS = {
				...
				Presence: _('Players score 10 DP per sector where they have at least 2 stars at game end')
			};
//
			this.DOMINATIONS = {
...
//
			...
			{
				if (!['Farmers', 'Slavers'].includes(dojo.getAttr(node, 'starpeople')))
				{
					dojo.connect(node, 'click', (event) =>
					{
						dojo.stopEvent(event);
		...
		focus: function (node)
		{
			...
			else
			{
				...
				dojo.style(node, {
					...
					'transform': `translate(${(window.innerWidth / zoom / 2 - rect.x - rect.width / 2)}px,${(window.innerHeight / zoom / 2 - rect.y - rect.height / 2)}px) scale(${Math.min(.90 * window.innerWidth / rect.width, .90 * window.innerHeight / rect.height) / zoom})`,
